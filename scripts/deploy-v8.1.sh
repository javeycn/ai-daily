...
# AI Daily v8.1 部署脚本
# 用法：
# 1. 本地: scp -P YOUR_PORT /tmp/aidaily-v8.1-assets.tar.gz YOUR_USER@YOUR_SERVER_IP:/tmp/
# 2. 服务器: sudo bash /tmp/deploy-v8.1.sh

set -euo pipefail

SITE_DIR="/data/web/www/ai-daily"
BACKUP_DIR="/data/web/www/ai-daily-bak-$(date +%Y%m%d%H%M%S)"
TMP_TAR="/tmp/aidaily-v8.1-assets.tar.gz"

echo "=== AI Daily v8.1 部署 ==="
...
# daily/ 目录只删除 HTML 壳子，保留数据（如果有的话被 data/ 管理）
# 注意：服务器上 daily/ 是 SSG 页面目录，也需要更新
...
echo "=== 验证 ==="
echo "--- data 完整性 ---"
ls "$SITE_DIR/data/daily/" 2>/dev/null | sort || echo "(无 data 目录)"
echo "--- _next build ---"
...
echo ""
echo "=== 部署完成 ==="
echo "⚠️  归档页现在通过 CSR 动态加载 data/index.json，不再依赖 SSG 数据"
